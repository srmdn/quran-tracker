...
// Starting page for each surah in the standard 604-page Medina Mushaf (index 0 = surah 1)
...
// Helper: estimate which Mushaf page a given surah:ayah falls on.
// Uses linear interpolation within the surah based on ayah count.
// Returns an approximate page number (standard 604-page Medina Mushaf).
export function getPageForPosition(surahNumber: number, ayahNumber: number): number {
  const startPage = SURAH_PAGES[surahNumber - 1];
  const nextSurahPage = surahNumber < 114 ? SURAH_PAGES[surahNumber] : 605;
  const totalSurahPages = nextSurahPage - startPage;
  const totalAyahs = SURAHS[surahNumber - 1].totalAyahs;
  const pageOffset = Math.floor(((ayahNumber - 1) / totalAyahs) * totalSurahPages);
